    ...
    try {
      ...
      return response.data.map((x) => {
        return {
          address: x.name,
          coordinates: {
            lat: x.address?.point?.lat,
            lng: x.address?.point?.lon,
          },
        };
      });
    } catch (error) {
      console.error(
        "Error in GeolocationService.search:",
